...
async function listStorageObjects(bucket, prefix = '') {
  ...
  try {
    const url = `${baseUrl}/storage/v1/object/list/${encodeURIComponent(bucket)}?prefix=${encodeURIComponent(prefix)}`;
    ...
    if (!resp.ok) throw new Error(data?.message || resp.statusText || 'Failed to list storage objects');
    return data;
  } catch (err) {
    if (err.name === 'AbortError') throw new Error('Supabase storage list timed out');
    throw err;
  } finally {
    clearTimeout(timeout);
  }
...
async function fetchTickets() {
  return supabaseRequest(
    'tickets?select=id,name,email,phone,ticket_type,quantity,amount,status,reference,requested_at,confirmed_at&status=eq.paid'
  );
}
